...
// Scheme return the scheme supported by this resolver.
func (r *Resolver) Scheme() string {
	return r.schema
}

// Build creates a new resolver.Resolver fo the given target
func (r *Resolver) Build(target resolver.Target, cc resolver.ClientConn, opt resolver.BuildOptions) (resolver.Resolver, error) {
	r.cc = cc
...
func (r *Resolver) start() (chan<- struct{}, error) {
	...
	r.cli, err = clientv3.New(clientv3.Config{
		...
		DialTimeout: time.Duration(r.DialTimeout) * time.Second * 5,
	})
	if err != nil {
		return nil, err
	}
...
func (r *Resolver) update(events []*clientv3.Event) {
	for _, ev := range events {
		...
		case clientv3.EventTypePut:
			...
			addr := resolver.Address{Addr: info.Addr, Metadata: info.Weitht}
